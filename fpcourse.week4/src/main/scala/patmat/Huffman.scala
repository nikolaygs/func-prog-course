...
object Huffman {
   ...
   */
    def decode(fullCodeTree: CodeTree, bits: List[Bit]): List[Char] = {

      def decodeChar(codeTree: CodeTree, bits: List[Bit]): (Char, List[Bit]) = codeTree match {
        case l: Leaf => (l.char, bits)
        case f: Fork => 
          if (bits.head == 0) decodeChar(f.left, bits.tail) 
          else decodeChar(f.right, bits.tail)
      }

      def decodeCharArray(codeTree: CodeTree, bits: List[Bit]): List[Char] = {
        if (bits.isEmpty) Nil
        else {
          val (head, remaning) = decodeChar(codeTree, bits) 
          val tail = if (remaning.isEmpty) Nil else decodeCharArray(fullCodeTree, remaning)
  
          head :: tail
        }
      }

      decodeCharArray(fullCodeTree, bits)
    }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      ...
      def containsChar(tree: CodeTree, char: Char) = tree match {
   ...
   */
    ...
    def codeBits(table: CodeTable)(char: Char): List[Bit] = table match {
      case Nil => throw new Exception // only if the character is not contained into the table
      case x :: xs => {
        val (c, bits) = x
        if (c == char) bits
